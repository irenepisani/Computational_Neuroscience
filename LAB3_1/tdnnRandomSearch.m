...
%% Function for TDNN model selection with random search over TDNN hyperparamters space
...
% tdnnRandomSearch()
    ...
    % DESCRIPTION
        % Use this function to execute TDNN model selection with Random
...
function [optParams, optVL, optTR] = tdnnRandomSearch(trX, trY, vlX, vlY, numTrials)
...
for i = 1: numTrials
    ...
    params = tdnnRandomConfig();
    
    % train a TDNN using TR set with that configuration and evaluate the
    % model on VL set
    [~, errTR,~, errVL, ~, ~] = tdnn(params, trX, trY, vlX, vlY);
